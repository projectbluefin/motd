	...
	for _, p := range preset {
		...
		case "aurora":
			tips = append(tips, []string{
				...
				l.Get("Like dinosaurs? Try `ujust toggle-dinosaurs`."),
			}...)
		case "bazzite":
			tips = append(tips, []string{
		...
		case "bluefin":
			tips = append(tips, []string{
				...
				l.Get("Almost all the wallpapers are made by Jacob Schnurr — [check out his Etsy shop](%s)", "https://www.etsy.com/shop/JSchnurrCommissions"),
			}...)
		case "deck":
			tips = append(tips, []string{
		...
		case "gnome":
			tips = append(tips, []string{
				...
				l.Get("Miss the wobbly windows from the early 2000s? Find the `Compiz windows effect` in the [Extension Manager](%s)", "appstream://com.mattjakeman.ExtensionManager"),
		...
		case "ublue":
			tips = append(tips, []string{
				...
				l.Get("The app store is made by Kolunmi — [support their work](%s)", "https://ko-fi.com/kolunmi"),
				...
				l.Get("Love the wallpapers? Check out the full [Universal Blue artwork collection](%s)", "https://docs.projectbluefin.io/artwork/"),
			}...)
		case "ublue-dev":
			tips = append(tips, []string{
